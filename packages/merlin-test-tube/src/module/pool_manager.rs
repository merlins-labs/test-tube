use merlin_std::types::merlin::poolmanager::v1beta1::{
    ...
    R: Runner<'a>,
{
    ...
    fn_query! {
        pub query_swap_exact_amount_in ["/merlin.poolmanager.v1beta1.Query/EstimateSwapExactAmountIn"]: EstimateSwapExactAmountInRequest => EstimateSwapExactAmountInResponse
    }

    // estimate single pool swap exact amount in
    fn_query! {
        pub query_single_pool_swap_exact_amount_in ["/merlin.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountIn"]: EstimateSinglePoolSwapExactAmountInRequest => EstimateSwapExactAmountInResponse
    }

    // estimate swap exact amount out
    fn_query! {
        pub query_swap_exact_amount_out ["/merlin.poolmanager.v1beta1.Query/EstimateSwapExactAmountOut"]: EstimateSwapExactAmountOutRequest => EstimateSwapExactAmountOutResponse
    }

    // estimate single pool swap exact amount out
    fn_query! {
        pub query_single_pool_swap_exact_amount_out ["/merlin.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountOut"]: EstimateSinglePoolSwapExactAmountOutRequest => EstimateSwapExactAmountOutResponse
    }

    // query num pools
    fn_query! {
        pub query_num_pools ["/merlin.poolmanager.v1beta1.Query/NumPools"]: NumPoolsRequest => NumPoolsResponse
    }

    // query all pools
    fn_query! {
        pub query_all_pools ["/merlin.poolmanager.v1beta1.Query/AllPools"]: AllPoolsRequest => AllPoolsResponse
    }

    // query pool
    fn_query! {
        pub query_pool ["/merlin.poolmanager.v1beta1.Query/Pool"]: PoolRequest => PoolResponse
    }

    // query spot price
    fn_query! {
        pub query_spot_price ["/merlin.poolmanager.v1beta1.Query/SpotPrice"]: SpotPriceRequest => SpotPriceResponse
    }

    // query total liquidity
    fn_query! {
        pub query_total_liquidity ["/merlin.poolmanager.v1beta1.Query/TotalPoolLiquidity"]: TotalPoolLiquidityRequest => TotalPoolLiquidityResponse
    }
}
...
mod tests {
    ...
    use crate::{
        ConcentratedLiquidity, Gamm, GovWithAppAccess, Module, MerlinTestApp, PoolManager,
        TokenFactory,
    };
    use std::str::FromStr;
    ...
    fn test_pool_manager() {
        ...
        let signer = app
            .init_account(&[Coin::new(1_000_000_000_000_000, "ufury")])
            .unwrap()
            .with_fee_setting(test_tube::account::FeeSetting::Auto {
                gas_price: Coin::new(25, "ufury"),
                gas_adjustment: 1.2,
            });
        ...
        gov.propose_and_execute(
            ...
            CreateConcentratedLiquidityPoolsProposal {
                title: "Create concentrated ufury:usdc pool".to_string(),
                description: "Create concentrated ufury:usdc pool, so that we can trade it"
                    .to_string(),
                pool_records: vec![PoolRecord {
                    denom0: denom0.clone(),
                    denom1: "ufury".to_string(),
                    tick_spacing: 100,
        ...
        let res = pool_manager
            .query_spot_price(&SpotPriceRequest {
                pool_id: 2,
                base_asset_denom: denom0.clone(),
                quote_asset_denom: "ufury".to_string(),
            })
            .unwrap();
        ...
        let trader = app
            .init_account(&[
                Coin::new(1_000_000_000, denom0.clone()),
                Coin::new(1_000_000_000, "ufury".to_string()),
            ])
            .unwrap();
