...
const FEE_DENOM: &str = "ufury";
const OSMO_ADDRESS_PREFIX: &str = "osmo";
const CHAIN_ID: &str = "merlin-1";
const DEFAULT_GAS_ADJUSTMENT: f64 = 1.2;

#[derive(Debug, PartialEq)]
pub struct MerlinTestApp {
    inner: BaseApp,
}

impl Default for MerlinTestApp {
    fn default() -> Self {
        MerlinTestApp::new()
    }
}
...
impl<'a> Runner<'a> for MerlinTestApp {
...
#[cfg(test)]
mod tests {
    use merlin_std::types::cosmos::bank::v1beta1::QueryAllBalancesResponse;
    use prost::Message;
    use std::option::Option::None;

    use cosmrs::Any;
    use cosmwasm_std::{attr, coins, Coin};
    use merlin_std::types::cosmos::bank::v1beta1::QueryAllBalancesRequest;

    use merlin_std::types::merlin::gamm::v1beta1::QueryTotalSharesRequest;
    use merlin_std::types::merlin::lockup::{
        self, MsgForceUnlock, MsgForceUnlockResponse, MsgLockTokens, MsgLockTokensResponse,
    };
    use merlin_std::types::merlin::tokenfactory::v1beta1::{
        MsgCreateDenom, MsgCreateDenomResponse, QueryParamsRequest, QueryParamsResponse,
    };

    use crate::module::Gamm;
    use crate::module::Wasm;
    use crate::runner::app::MerlinTestApp;
    use crate::Bank;
    ...
    fn test_init_accounts() {
        ...
        let accounts = app
            .init_accounts(&coins(100_000_000_000, "ufury"), 3)
            .unwrap();
    ...
    fn test_execute() {
        ...
        let acc = app
            .init_account(&coins(100_000_000_000_000, "ufury"))
            .unwrap();
    ...
    fn test_custom_fee() {
        ...
        let alice = app.init_account(&coins(initial_balance, "ufury")).unwrap();
        let bob = app.init_account(&coins(initial_balance, "ufury")).unwrap();

        let amount = Coin::new(1_000_000, "ufury");
    ...
    fn test_param_set() {
        ...
        let whitelisted_users = app
            .init_accounts(&coins(1_000_000_000_000, "ufury"), 2)
            .unwrap();
    ...
    fn test_set_param_set() {
        ...
        let pool_id = gamm
            .create_basic_pool(
                &[Coin::new(1_000_000, "ufury"), Coin::new(1_000_000, "uion")],
                &whitelisted_user,
            )
            .unwrap()
            .data
        ...
        app.execute::<_, MsgLockTokensResponse>(
            MsgLockTokens {
                ...
                duration: Some(merlin_std::shim::Duration {
                    seconds: 1000000000,
                    nanos: 0,
                }),
                coins: shares,
