...
import (
	...
	concentrateliquiditytypes "github.com/merlins-labs/merlin/v16/x/concentrated-liquidity/types"
...
func SetupMerlinApp() *app.MerlinApp {
	db := dbm.NewMemDB()
	appInstance := app.NewMerlinApp(
		log.NewNopLogger(),
		db,
		nil,
		true,
	...
	poolIncentivesParams.MintedDenom = "ufury"
	...
	// replace sdk.DefaultDenom with "ufury", a bit of a hack, needs improvement
	stateBytes = []byte(strings.Replace(string(stateBytes), "\"stake\"", "\"ufury\"", -1))
...
func (env *TestEnv) BeginNewBlock(executeNextEpoch bool, timeIncreaseSeconds uint64) {
	...
	if len(validators) >= 1 {
		...
		err := simapp.FundAccount(env.App.BankKeeper, env.Ctx, valAddrFancy.Bytes(), sdk.NewCoins(sdk.NewInt64Coin("ufury", 9223372036854775807)))
		if err != nil {
...
func (env *TestEnv) beginNewBlockWithProposer(executeNextEpoch bool, proposer sdk.ValAddress, timeIncreaseSeconds uint64) {
	...
	header := tmtypes.Header{ChainID: "merlin-1", Height: env.Ctx.BlockHeight() + 1, Time: newBlockTime}
