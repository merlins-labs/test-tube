...
fn main() {
    ...
    let prebuilt_lib_dir = manifest_dir.join("libmerlintesttube").join("artifacts");

    let lib_name = "merlintesttube";

    let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());

    let header = if std::env::var("DOCS_RS").is_ok() {
        manifest_dir
            .join("libmerlintesttube")
            .join("artifacts")
            .join("libmerlintesttube.docrs.h")
    } else {
        out_dir.join(format!("lib{}.h", lib_name))
    };
    // rerun when go code is updated
    println!("cargo:rerun-if-changed=./libmerlintesttube");
...
fn build_libmerlintesttube(out: PathBuf) {
    ...
    let exit_status = Command::new("go")
        .current_dir(manifest_dir.join("libmerlintesttube"))
        .arg("build")
